...
@app.route('/Chlamidya_and_gonorrhea')
def Chlamidya_and_gonorrhea():
    return render_template('Chlamidya_and_gonorrhea.html')
...
def register_client():
    ...
    if request.method=='POST' and 'unique_id' in request.form:
        ...
        with engine.connect() as con:
            ...
            con.execute(text(f"INSERT INTO Chlamidya_and_gonorrhea(created_by, created_at,updated_at, updated_by, unique_id))\
                                      VALUES('{created_by}','{created_at}', '{updated_at}', '{updated_by}','{unique_id}')"))
